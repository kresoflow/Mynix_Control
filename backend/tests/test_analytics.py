...
async def test_analytics_endpoints_and_permissions(async_client: AsyncClient, db_session):
    ...
    tenant = Tenant(name="Analytics Cafe", schema_name="tenant_analytics", is_active=True)
    db_session.add(tenant)
    await db_session.commit()
    await db_session.refresh(tenant)

    # Permission
    perm = Permission(name="analytics:view", description="View analytics and dashboards")
    db_session.add(perm)
    await db_session.commit()
    ...
    dash_res = await async_client.get("/dashboard/today", headers=manager_headers)
    ...
    metrics_res = await async_client.get("/analytics/metrics?period=today", headers=manager_headers)
    ...
    xray_res = await async_client.get("/analytics/xray?period=today", headers=manager_headers)
    ...
    forbidden_dash = await async_client.get("/dashboard/today", headers=cashier_headers)
    assert forbidden_dash.status_code == 403

    forbidden_metrics = await async_client.get("/analytics/metrics", headers=cashier_headers)
